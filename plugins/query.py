...
async def cb_handler(client: Bot, query: CallbackQuery):
    ...
    if data == "close":
        ...
        await query.message.edit_text(
            text=(
                f"<b>○ Updates : <a href='https://t.me/Boby_Vip'>Rohit</a>\n"
        ...
        await client.send_photo(
            ...
            caption=(
                ...
                f"● 😋കഴപ്പൻ ലീക്കഡ്  തുണ്ട് ഫുൾ കളക്ഷൻ😍 DAILY UPDATION😍💋💋  #ALLCOLLECTIONS\n\n"
                f"● 😍💋മല്ലു മുസ്ലിം സ്പെഷ്യൽ കളക്ഷൻസ്💥💦  ⭐️FOR SUBSCRIPTIONS🎖\n\n"
                f"● 👤 @BOBY_VIP ✅\n\n"
                f"● 👤 @BOBY_VIP ✅\n\n\n"
    ...
    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('Back', callback_data='set_shortener')]])
        ...
        if await authoUser(query, query.from_user.id, owner_only=True):
            ...
            # Edit the same message instead of sending a new one
                ...
                await query.message.edit_text(
                    f"<b>! Error Occurred..\n<blockquote>Reason:</b> {e}</blockquote><b><i>Contact developer: @Boby_Vip</i></b>",
                    reply_markup=InlineKeyboardMarkup(
                        [[InlineKeyboardButton("Close ✖️", callback_data="close")]]
                    )
                )
